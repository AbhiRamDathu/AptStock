...
Use this before your next supplier order.
      ...
      <div style={{
        backgroundColor: hasAnyCurrentStockInActions() ? '#dcfce7' : '#fff7ed',
        color: hasAnyCurrentStockInActions() ? '#166534' : '#9a3412',
        padding: '10px 14px',
        borderRadius: '999px',
        fontSize: '13px',
        fontWeight: '800',
    ...
    <div style={{
      backgroundColor: hasAnyCurrentStockInActions() ? '#f0fdf4' : '#fff7ed',
      border: hasAnyCurrentStockInActions() ? '1px solid #bbf7d0' : '1px solid #fed7aa',
      color: hasAnyCurrentStockInActions() ? '#166534' : '#9a3412',
      padding: '14px 16px',
      borderRadius: '12px',
      fontSize: '14px',
...
<div style={{
  ...
  {getDeadStockRiskItems().length === 0 ? (
  <div style={{
    padding: '22px',
    backgroundColor: '#f8fafc',
    border: '1px solid #e2e8f0',
    ...
    No dead-stock suspects found in this data. Check slow-moving items for watchlist products.
  </div>
) : (
    <div style={{
      display: 'grid',
      ...
      {getDeadStockRiskItems().map((item, index) => {
        const movement = item._movement;
        const clearanceDays = item._clearanceDays;
        const blockedCash = item._blockedCash;
        const daily = item._dailyMovement;
        const hasStock = hasCurrentStockForAction(item);
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ backgroundColor: '#f9fafb', padding: '10px', borderRadius: '10px' }}>
                ...
                <div style={{ fontSize: '18px', color: '#111827', fontWeight: '900' }}>
                  {daily.toFixed(2)}
                </div>
              </div>

              <div style={{ backgroundColor: movement.bg, padding: '10px', borderRadius: '10px' }}>
                <div style={{ fontSize: '12px', color: movement.color, fontWeight: '700' }}>
                  Movement Status
                </div>
                <div style={{ fontSize: '15px', color: movement.color, fontWeight: '900' }}>
                  {movement.label}
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#f9fafb', padding: '10px', borderRadius: '10px' }}>
                ...
                <div style={{ fontSize: '18px', color: '#111827', fontWeight: '900' }}>
                  {hasStock ? Number(item.current_stock).toLocaleString('en-IN') : 'Not provided'}
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#f9fafb', padding: '10px', borderRadius: '10px' }}>
                ...
                <div style={{
                  fontSize: '18px',
                  color: clearanceDays && clearanceDays >= 60 ? '#b91c1c' : '#111827',
                  fontWeight: '900'
                }}>
                  {clearanceDays ? `${clearanceDays} days` : 'Need stock'}
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              {hasStock && blockedCash
                ? `Possible cash tied in this item: ${formatMoneyShort(blockedCash)}. Do not repeat order blindly unless movement improves.`
                : `Dead-stock suspect: very weak movement detected. Add current stock or purchase quantity to calculate exact blocked cash.`}
            </div>
            ...
            <div style={{
              ...
              {daily <= 1
                ? 'Do not reorder now. Move to offer/display area and ask supplier for scheme, replacement, or support.'
                : daily <= 4
                  ? 'Avoid repeat order now. Move to offer/display area and ask supplier for scheme, replacement, or support.'
                  : 'Monitor only. Movement is acceptable compared with other risk items.'}
            </div>
            ...
            <div style={{
              ...
              Recommended Qty: {recommendedQty ? recommendedQty.toLocaleString('en-IN') : 'N/A'} • Source: {item.datasource || item.data_source || 'Sales data'}
            </div>
          </div>
        );
      })}
...
<div style={{
  ...
  <div style={{
    ...
    Showing {getFilteredPriorityActions().length} of {data.priorityActions.length} owner actions
